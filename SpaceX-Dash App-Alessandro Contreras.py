...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        # Filtrar el DataFrame para incluir solo datos del sitio de lanzamiento seleccionado
...
def get_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        filtered_df_2 = spacex_df[(spacex_df['Payload Mass (kg)'] >= payload_range[0]) & (spacex_df['Payload Mass (kg)'] <= payload_range[1])]
        fig_2 = px.scatter(filtered_df_2,
                         x="Payload Mass (kg)",
                         y="class",
                         color="Booster Version Category",
                         title="Correlation between Payload Mass and Launch Success for All Sites")
    else:
        filtered_df_2 = spacex_df[(spacex_df['Launch Site'] == selected_site) & 
